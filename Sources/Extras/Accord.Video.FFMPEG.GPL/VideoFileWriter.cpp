...
#pragma endregion
            ...
            {
                Open(fileName, width, height, frameRate, codec, 1000000);
            }
                ...
                AudioCodec audioCodec, int audioBitRate, int sampleRate, int channels)
            {
                ...
                try
                {
					...
					libffmpeg::av_opt_set(data->FormatContext->priv_data, "preset", "slow", 0);
					libffmpeg::av_opt_set(data->FormatContext->priv_data, "crf", "24", 0);
...
;
						...
						//packet.dts = data->VideoFrame->pts;
						//packet.pts = data->VideoFrame->pts;
						// write the compressed frame to the media file
						ret = libffmpeg::av_interleaved_write_frame(data->FormatContext, &packet);
                    }
                    else
     ...
     //               codecContex->max_b_frames = 0;
            ...
            void open_video(WriterPrivateData^ data)
            {
				...
				libffmpeg::av_dict_set(&param, "preset", "slow", 0);
				libffmpeg::av_dict_set(&param, "tune", "zerolatency", 0);
				libffmpeg::av_dict_set(&param, "profile", "main", 0);
				libffmpeg::av_dict_set(&param, "cqp", "50", 0);
				// open the codec 
            ...
            void open_audio(WriterPrivateData^ data)
            {
                ...
                // Open it.
                if (libffmpeg::avcodec_open2(codecContext, codec,NULL) < 0)
                {
